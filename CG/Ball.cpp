...
struct Ball {
	typedef Vector3D::ValType ValType;
	Vector3D c;
	ValType r;
	Ball() : c(), r() { }
...
Ball smallest_ball(vector<Vector3D> & points) {
	...
	Vector3D c = points[0];
	Vector3D::ValType r = Vector3D::ValType(); // 0
	for (int i=1; i<SIZE; ++i) {
        if (is_nsml(r, dist(points[i], c))) {
        	continue;
        }
		c = points[i];
		r = Vector3D::ValType(); // 0
		for (int j=0; j<i; ++j) {
...
Ball::ValType volume_union(Ball b1, Ball b2) {
	Ball::ValType distance = dist(b1.c, b2.c);
	if (is_nsml(distance, b1.r + b2.r)) {
		return 4.0 / 3.0 * PI * (pow(b1.r, 3.0) + pow(b2.r, 3.0));
	}
	Ball::ValType max_r = max_by_val(b1.r, b2.r);
	Ball::ValType min_r = min_by_val(b1.r, b2.r);
	if (is_nsml(max_r, min_r + distance)) {
		return 4.0 / 3.0 * PI * pow(max_r, 3.0);
	}
	Ball::ValType h1 = (distance + (b1.r*b1.r - b2.r*b2.r) * (1.0 / distance)) * 0.5;
	Ball::ValType h2 = (distance - (b1.r*b1.r - b2.r*b2.r) * (1.0 / distance)) * 0.5;
	return PI * (
