...
std::vector<int> KMP_build_next(std::string const & P) {
	std::vector<int> next(P.length()+1, -1);
	int ni = -1;
	int pi = 0;
	while (pi < P.length()) {
		if (ni == -1 || P[pi] == P[ni]) {
...
int KMP_match(std::string const & T, std::string const & P) {
	std::vector<int> next = KMP_build_next(P);
	int ti = 0;
	int pi = 0;
